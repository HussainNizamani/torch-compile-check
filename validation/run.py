...
compile-check on public, CPU-runnable models -- rather than the fixture-
...
script's job is only to run ``compile-check`` on each one, record what came
...
class TargetSpec:
    ...
    :func:`importlib.util.find_spec` before invoking ``compile-check`` at
...
def run_target(spec: TargetSpec) -> TargetResult:
    """Run one target through ``compile-check``, or skip it cleanly."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    update_readme(
        ...
        "compile-check is run against public, CPU-runnable models "
