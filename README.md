...
Bring your own model. Point compile-check at your `(callable, weights,
...
`torch.compile` is a compiler; compile-check is differential testing for it,
...
```console
$ pip install git+https://github.com/HussainNizamani/compile-check.git
```
...
Once compile-check ships to PyPI:

```console
$ pip install compile-check
```
...
compile-check run against public, CPU-runnable models to measure the
...
- the module is registered in `sys.modules` under the file's stem, so a
  ...
  `_compile_check_target_<stem>` instead, rather than shadowing it.

Both happen in the `compile-check` process only. Nothing is written to your
...
It installs `compile-check`, runs it against the entrypoints you declare,
...
old `ref`: the CLI's main run path landed in M1-3, so `compile-check
...
tool. If eager and compiled agree on the wrong answer, compile-check
...
compile-check productizes the workflow the PyTorch team does by hand
...
compile-check audits the model the user already has.
