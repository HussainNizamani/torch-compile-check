...
:mod:`compile_check.discover` resolves a target, :mod:`compile_check.runner`
runs it under every backend, all five oracles of PLAN.md "Oracles" compare the
lanes, :mod:`compile_check.localize` names the compilation stage a divergence
first appears in, :mod:`compile_check.minimize` shrinks a reproducing case, and
:mod:`compile_check.report` writes the terminal report, the JSON artifact, the
